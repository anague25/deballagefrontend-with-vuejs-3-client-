<template>
    <main>
        <PageHeader>
            ...
            <template #subtitle>Tous les utilisateurs</template>
        </PageHeader>
        <Loader></Loader>
        <!-- Main page content-->
        <div class="container-xl px-4 mt-n10">
            ...
            <div class="card mb-4">
                <div class="card-header">Gerer les Utilisateurs</div>
                <div class="card-body">
                    <div class="dataTable-wrapper no-footer fixed-columns">
                        <div class="dataTable-top">
                            <router-link class="btn btn-primary btn-sm" to="/dashboard/users/create">Create un
                            ...
                            <table class="dataTable-table">
                                ...
                                <tbody>
                                    <tr v-for="user in filterUsers" :key="user.id">
                                        ...
                                        <td>{{ user.state }}</td>
                                        <td>{{ user.email }}</td>
                                        <td>{{ user.role ?? 'Normaly User' }}</td>

                                        <td>
                                            <img :src="path + user.profile" alt="user Image" width="50" height="50">
                                        </td>
                                        <td>
...
export default {
    ...
    setup() {
        ...
        const fetchUsers = async (page = 1) => {
            ...
            try {
                await store.dispatch('users/fetchUsers', { page: page, search: searchInput.value });
        ...
        const { users, totalPages, currentPage, elementsPerPage, totalElements } = toRefs(store.state.users);
        ...
        const filterUsers = computed(() => {
            ...
            return users.value.filter(user => {
                return user.firstName && user.firstName.toLowerCase().includes(searchQuery);
            });
        });
        ...
        onMounted(async () => {
            ...
            if (route.query.success === 'true') {
                Notification.success('Users updated successfully');
                setTimeout(() => {
